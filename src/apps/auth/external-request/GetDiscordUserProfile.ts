...
export default async function getDiscordUserProfile({
  ...
  try {
    ...
    console.error('------ GET_DISCORD_USER_PROFILE_ERROR ------');
    console.error(err.response.data.error);
    console.error('------ ------------------------------ ------');

    const status = err.response.status as number;
    if (status === 401) {
      throw new UnauthorizedException('GET_DISCORD_USER_PROFILE_AUTH_ERROR');
    }

    throw new InternalServerErrorException('GET_DISCORD_USER_PROFILE_SERVER_ERROR');
  }
}
