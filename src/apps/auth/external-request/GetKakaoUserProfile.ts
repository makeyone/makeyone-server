...
export default async function getKakaoUserProfile({ accessToken }: GetKakaoUserProfileInput): Promise<GetKakaoUserProfileOutput> {
  try {
    ...
    console.error('------ GET_KAKAO_USER_PROFILE_ERROR ------');
    console.error(err.response.data);
    console.error('------ ---------------------------- ------');

    const status = err.response.status as number;
    if (status === 401) {
      throw new UnauthorizedException('GET_KAKAO_USER_PROFILE_AUTN_ERROR');
    }

    throw new InternalServerErrorException('GET_KAKAO_USER_PROFILE_SERVER_ERROR');
  }
}
